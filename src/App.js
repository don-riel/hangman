...
function App() {
  ...
  const increaseCorrectCount = () => {
    ...
    if (correctCount === uniqueLetterArr.length - 1) {
      // used "uniqueLetterArr.length -1" because updating setCorrectCount is async
  ...
  const increaseWrongCount = () => {
    ...
    if (wrongCount === 5) {
      //accepted wrong count 6, setWrongCount is async so 5 is used, alternative is using useEffect
      setgameStatus(!gameStatus);
    }
  };
